                    ...
                    <form class="fn_variants" action="/{$lang_link}cart">
                        <div class="row">
                            <div class="col-sm-6">
                                {* Product variants *}
                                {if $product->radio}
                                    <div class="{if $product->variants|count < 2} hidden{/if}">
                                        {foreach $product->variants as $v}
                                            <div>
                                                <label name="variant" class="fn_variant">                                            
                                                    <input{if $smarty.get.variant == $v->id} checked{/if} value="{$v->id}" data-price="{$v->price|convert}" data-stock="{$v->stock}"{if $v->compare_price > 0} data-cprice="{$v->compare_price|convert}"{/if}{if $v->sku} data-sku="{$v->sku|escape}"{/if} {if $v->units}data-units="{$v->units}"{/if} type="radio" name="variant" class="selected"/>
